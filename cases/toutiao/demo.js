...
//
// 关键点：
// 1) init 必须传 paths，否则 SDK 不会给页面请求补 a_bogus（实测传空即不签名）；
// 2) a_bogus 是 SDK 在内部 hook 的 fetch / XMLHttpRequest 上同步算出来的，
//    这里通过 window.fetch 桩（__capturedRequests）把签名后的 URL 取出来；
// 3) msToken 由 SDK 向 https://mssdk.bytedance.com/web/common?ms_appid=24 请求，
//    从响应头 x-ms-token 取到后缓存在 localStorage['xmst']（纯字符串）。
...
window.getSignedUrl = async function(url) {
    __capturedRequests.length = 0
    await window.fetch(url, {})
    if (!__capturedRequests.length) return url
    return __capturedRequests[__capturedRequests.length - 1].url
}

// 只取 a_bogus
window.getABogus = async function(url) {
    var signed = await window.getSignedUrl(url)
...
// 生成最终请求参数：msToken 必须在签名之前拼进 query（a_bogus 覆盖整个 query 串）
window.buildParams = async function(url) {
    var msToken = window.getMsToken()
    var u = new URL(url)
    if (msToken && !u.searchParams.get('msToken')) u.searchParams.set('msToken', msToken)
    var signed = await window.getSignedUrl(u.toString())
    return {
        msToken: msToken,
